...
(function() {
  ...
  const all$2 = (handlers, f) => (...args) =>
    foldl(
      handlers,
      (acc, handler) => acc && f(handler).apply(undefined, args),
      true
    );
  const any = (handlers, f) => (...args) =>
    foldl(
      handlers,
      (acc, handler) => acc || f(handler).apply(undefined, args),
      false
  ...
  const getTrace = () => {
    ...
    if (err.stack !== undefined) {
      ...
      return find$5(
        lines,
        line =>
          line.indexOf("alloy") > 0 && !exists(path, p => line.indexOf(p) > -1)
      ).getOr(unknown);
    } else {
      return unknown;
    }
  ...
  const withoutReuse = (parent, data) => {
    preserve$1(() => {
      replaceChildren(parent, data, () =>
        map$2(data, parent.getSystem().build)
      );
    }, parent.element);
  };
  ...
  const fixedToolbarContainerTarget = option$2(
  ...
  const containerAlignRightClass =
  ...
  const renderCollection = (spec, providersBackstage, initialData) => {
    ...
    const iterCollectionItems = (comp, applyAttributes) =>
      map$2(
        descendants(comp.element, ".tox-collection__item"),
        applyAttributes
      );
      ...
      behaviours: SketchBehaviours.augment(detail.sandboxBehaviours, [
        ...
        Sandboxing.config({
          ...
          isPartOf: (container, data, queryElem) => {
            return (
              isPartOf$1(data, queryElem) || isPartOf$1(hotspot, queryElem)
            );
          },
          getAttachPoint: () => {
  ...
  const hasEdge = (detail, edgeName) =>
    detail[edgeName + "-edge"] !== undefined;
  ...
  const handleMovement = (direction, isVerticalMovement) => (
    spectrum,
    detail
  ...
  const middleX = (anchor, element) =>
    anchor.x + anchor.width / 2 - element.width / 2;
  const eastEdgeX = (anchor, element) =>
    anchor.x + anchor.width - element.width;
  ...
  const preserve = (anchor, element, bubbles, placee, bounds) => {
    const layout = getPlacement(placee)
      .map(lookupPreserveLayout)
      .getOr(north);
  ...
  const getUrlPicker = (editor, filetype) =>
    getPickerSetting(editor, filetype).map(picker => entry =>
      Future.nu(completer => {
  ...
  const getPrior = (elem, state) =>
    state
      .getInitialPos()
      .map(pos => bounds(pos.bounds.x, pos.bounds.y, get$c(elem), get$d(elem)));
  ...
  const isOpen = (toolbar, detail) =>
    getPart(toolbar, detail, "overflow")
      .map(Sliding.hasGrown)
      .getOr(false);
  ...
  const getSpec$4 = editor => {
    ...
    const onAction = rawItem => () =>
      find$5(
        alignMenuItems,
        item => item.format === rawItem.format
  ...
  const renderFromBridge = (bridgeBuilder, render) => (
    spec,
    extras,
    editor
  ) => {
      ...
      switch (getToolbarLocation(editor)) {
        case ToolbarLocation.auto:
          ...
          if (roomAtTop) {
            ...
            const roomAtBottom =
              targetBounds.bottom < docHeight - toolbarHeight;
            if (roomAtBottom) {
              return "bottom";
            } else {
              const winBounds = win();
      ...
      buttonBehaviours: derive$1([
        ...
        config("wordcount-events", [
          runOnExecute$1(comp => {
            ...
            const newMode =
              currentVal.mode === "words" ? "characters" : "words";
        ...
        components: [
          {
            dom: {
              ...
              attributes: {
                href:
                  "https://www.tiny.cloud/powered-by-tiny?utm_campaign=editor_referral&utm_medium=poweredby&utm_source=tinymce&utm_content=v6",
                rel: "noopener",
                target: "_blank",
  ...
  const getMaxTabviewHeight = (dialog, tabview, tablist) => {
    ...
    const dialogBottomMargin =
      parseInt(get$e(dialog, "margin-bottom"), 10) || 0;
